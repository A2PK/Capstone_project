...
def preprocessData(data, elements_list, date_column_name):
    logger.info("Preprocessing data...")
    data.ffill()
    # date_column_name = 'Ngày quan trắc'
    ...
    columnToKeep = elements_list + ['Place','date','day','month','year']
    available_columns = [col for col in columnToKeep if col in data.columns]
    data = data[available_columns]
    logger.info("Preprocessing complete.")
    return data

def predictWithMLModel(file, num_step, freq_days, elements_list, date_column_name, place_id, date_tag, model_dir="saved_models"):
    ...
    df = preprocessData(df, elements_list, date_column_name)
...
def trainWithMLModel(file, elements_list, date_column_name, place_id, date_tag, train_test_ratio= 0.7, model_dir='saved_models'):
    NUM_LAGS = 12
    ...
    for model_type_name in model_type_list:
        ...
        model_path,eval_dict = train_export_model(df, elements_list, place_id, NUM_LAGS, train_split_ratio=train_test_ratio, base_model=base_model, base_model_name=model_type_name, model_dir=model_dir)
        if model_path:
